...
function updateInterview(client, interview, meta, cb) {
  if(meta) {
    getIndexingCommands(interview, "meta", function(err, commands) {
    ...
    client.search({
      ...
      getIndexingCommands(interview, "update", function(err, commands) {
        if (err) return cb(err);;
